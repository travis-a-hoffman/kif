...
public class FileProducer<K, V> implements Producer<K, V> {

  private final Cluster cluster;
  private final Partitioner partitioner;
  private final List<ProducerRecord<K, V>> sent;
  private final List<ProducerRecord<K, V>> uncommittedSends;
  private final Deque<FileProducer.Completion> completions;
  ...
  private final Map<MetricName, Metric> fileMetrics;
   ...
   */
  public FileProducer(final Cluster cluster,
  ...
  /**
   * Create a new file producer with invented metadata the given autoComplete setting and key\value serializers.
   *
   * Equivalent to {@link #FileProducer(Cluster, boolean, Partitioner, Serializer, Serializer)} new FileProducer(Cluster.empty(), autoComplete, new DefaultPartitioner(), keySerializer, valueSerializer)}
   */
  public FileProducer(final boolean autoComplete,
                      final Serializer<K> keySerializer,
                      final Serializer<V> valueSerializer) {
  ...
  /**
   * Create a new file producer with invented metadata the given autoComplete setting, partitioner and key\value serializers.
   *
   * Equivalent to {@link #FileProducer(Cluster, boolean, Partitioner, Serializer, Serializer)} new FileProducer(Cluster.empty(), autoComplete, partitioner, keySerializer, valueSerializer)}
   */
  public FileProducer(final boolean autoComplete,
                      final Partitioner partitioner,
                      final Serializer<K> keySerializer,
  ...
  /**
   * Create a new file producer with invented metadata.
   *
   * Equivalent to {@link #FileProducer(Cluster, boolean, Partitioner, Serializer, Serializer)} new FileProducer(Cluster.empty(), false, null, null, null)}
   */
  public FileProducer() {
  ...
  public void initTransactions() {
    ...
    if (this.transactionInitialized) {
      throw new IllegalStateException("FileProducer has already been initialized for transactions.");
    }
  ...
  private synchronized void verifyProducerState() {
    if (this.closed) {
      throw new IllegalStateException("FileProducer is already closed.");
    }
    if (this.producerFenced) {
      throw new ProducerFencedException("FileProducer is fenced.");
    }
  }

  private void verifyTransactionsInitialized() {
    if (!this.transactionInitialized) {
      throw new IllegalStateException("FileProducer hasn't been initialized for transactions.");
    }
  }
  ...
  public synchronized Future<RecordMetadata> send(ProducerRecord<K, V> record, Callback callback) {
    ...
    if (this.producerFenced) {
      throw new KafkaException("FileProducer is fenced.", new ProducerFencedException("Fenced"));
    }
    ...
    FileProducer.Completion completion = new FileProducer.Completion(offset, new RecordMetadata(topicPartition, 0, offset,
   ...
   */
  public void setFileMetrics(MetricName name, Metric metric) {
    fileMetrics.put(name, metric);
  }
  ...
  public synchronized boolean errorNext(RuntimeException e) {
    FileProducer.Completion completion = this.completions.pollFirst();
    if (completion != null) {
