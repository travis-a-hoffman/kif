...
/**
 * An implementation of the {@code Consumer} interface which consumes records from an {@code InputStream}.
 * <p/>
 * A StreamConsumer implementation may be used as a drop-in replacement for a KafkaProducer. This simplifies
 * writing applications which need to stream records from a variety of sources of Records. can be more easily configured to write to Kafka, Streams, or Local Files.
 * For example, this may be used directly to write records in their serialized form to an OutputStream.
 * <p/>
 * To simplify reading data from a variety of targets (Streams, LocalFiles, or Kafka Topics), KifCli provides
 * RecordStreams which wrap around any Consumer / Producer implementations.  This abstracts away much of the
 * details of writing records in the simple read/write case of KifCli.
 * <i> This class is <em>not</em> threadsafe.</i>
 * <pre>
 *  +----------------------+
 *  |     Application      | --> KifCli / KifGenCli app reads POJOs from {@link RecordOutputStream}
 *  +----------------------+
 *  |   DefaultRecordInputStream  | --> Reads {@link ConsumerRecord} from a {@link Consumer}, returns POJOs (Type T)
 *  +----------------------+
 *  |    StreamConsumer    | --> Reads byte[] from an InputStream, returns ConsumerRecord<K, V>
 *  +----------------------+
 *  |     InputStream      | --> Reads bytes from source, returns byte[].
 *  +----------------------+</pre>
 * <p/>
 * To write to {@code System.out}:
 * <pre>
 *  +----------------------+
 *  |     Application      | --> KifCli / KifGenCli application code; reads POJOs (Type T)
 *  +----------------------+
 *  |   DefaultRecordInputStream  | --> Reads {@link ConsumerRecord} from the StreamConsumer
 *  +----------------------+
 *  |    StreamConsumer    | --> Reads bytes[] from an InputStream
 *  +----------------------+
 *  |      System.in       | --> Reads bytes from source.
 *  +----------------------+</pre>
 * <p/>
 *
 ...
 */
public class StreamConsumer<K, V> implements Consumer<K,V> {
  ...
  public StreamConsumer() {
    // TODO Pass in offsetResetStrategy as parameter? See MockConsumer for details.
    this.subscriptions = new SubscriptionState(new LogContext(), OffsetResetStrategy.EARLIEST);
  ...
  @Override
  public synchronized void commitSync(Map<TopicPartition, OffsetAndMetadata> offsets, final Duration timeout) {
    commitSync(offsets);
  }
  ...
  @Override
  public List<PartitionInfo> partitionsFor(String topic, final Duration timeout) {
  ...
  public ConsumerGroupMetadata groupMetadata() {
    return new ConsumerGroupMetadata("stream.group.id", 1, "1", Optional.empty());
  }
  ...
  public synchronized void close() {
    close(RecordStreams.DEFAULT_CLOSE_TIMEOUT_MS, TimeUnit.MILLISECONDS);
  }
  ...
  @Override
  public synchronized Map<TopicPartition, Long> endOffsets(Collection<TopicPartition> partitions, Duration timeout) {
  ...
  /**
   * Used by MockConsumer to simulate records being added from Kafka Client
   * Used by StreamConsumer to buffer up records as they're read from the stream.
   */
  synchronized void addRecord(ConsumerRecord<K, V> record) {
    ensureNotClosed();
   ...
   */
  synchronized void rebalance(Collection<TopicPartition> newAssignment) {
  ...
  synchronized void updatePartitions(String topic, List<PartitionInfo> partitions) {
    ensureNotClosed();
  ...
  /**
   * Helper to check open/closed state. Throws IllegalStateException once the conusmer has been closed.
   */
  ...
  /**
   * Used to inject Exceptions encountered while {@code poll()}-ing.
   */
  synchronized void setPollException(KafkaException exception) {
    this.pollException = exception;
  }

  /**
   * Used to inject Exceptions encountered while retrieving offsets, results in exception thrown
   * from {@code beginningOffsets()}, {@code endOffsets()}
   */
  synchronized void setOffsetsException(KafkaException exception) {
    this.offsetsException = exception;
  }

  /**
   * Used by MockConsumer tests to simulate a rebalance is pending.
   */
   ...
   * Used by MockConsumer tests to simulate a rebalance is pending.
   */
  private void resetShouldRebalance() {
    shouldRebalance = false;
  }

  /**
   * Helper to track poll request timeouts
   */
