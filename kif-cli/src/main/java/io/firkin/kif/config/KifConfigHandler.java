...
public class KifConfigHandler implements ConfigHandler {
  private static final Logger log = LoggerFactory.getLogger(KifConfigHandler.class);

  private static final String DOT_KIF = ".kif";
  private static final String DEFAULT_CONFIG_FILE_NAME = "config.json";
  private static final String DEFAULT_KIF_CONFIG_DIR =
      System.getProperty("KIF_HOME", System.getProperty("user.home") + "/" + DOT_KIF);

  public static ConfigHandler of() {
    return of(Path.of(DEFAULT_KIF_CONFIG_DIR, DOT_KIF, DEFAULT_CONFIG_FILE_NAME));
  }

  // @VisibleForTest
  static ConfigHandler of(Path testConfigFilePath) {
    return new KifConfigHandler(testConfigFilePath);
  }

  // ------------------------------------------------------------------------------------------------------------------

  private KifConfigHandler(Path configPath) {
    if (configPath == null) {
    ...
    if (file.isDirectory()) {
      ...
      this.configFilePath = Path.of(file.getAbsolutePath(), DEFAULT_CONFIG_FILE_NAME);
    } else if (file.isFile()) {
